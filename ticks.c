...
long tapcycles(void){
  ...
  switch( tapstate ){
    case 0:
      fread(&blocklen, 2, 1, ft);
      tappos= 0;
      block++;
      fread(tapbuf, 1, blocklen, ft);
      tapstate++;
      tapcount= (tapbyte= tapbuf[tappos])<128 ? 8063+1000 : 3223+1000;
      return 2168;
    case 1:
      --tapcount || tapstate++;
      return 2168;
    ...
    case 16: case 17: case 18:
      return tapbyte<<(tapstate++-4>>1) & 128 ? 1710 : 855;
    case 19:
      t= tapbyte & 1 ? 1710 : 855;
      if( ++tappos==blocklen )
        tapstate= 0;
      else
        tapbyte= tapbuf[tappos],
        tapstate= 4;
      return t;
  }
}
...
int main (int argc, char **argv){
  ...
  if( argc==1 )
    printf("Ticks v0.13 beta, a silent Z80 emulator by Antonio Villena, 28 Nov 2012\n\n"),
    ...
    printf("  -tape <file>   emulates ZX tape in port $FE from a .TAP file\n"),
  ...
  if( size==65572 ){
    fread(&tapbyte, 1, 1, fh);
    fread(&tapstate, 1, 1, fh);
    ear= tapstate>>1 & 64 | 191;
    tapstate&= 127;
    fread(&block, 2, 1, fh);
    for ( t= 0; t<block; t++ )
      fread(&blocklen, 2, 1, ft),
  ...
  if( output ){
    ...
    if( !stricmp(strchr(output, '.'), ".sna" ) )
      ...
      fwrite(&mp, 2, 1, fh);   // 1001c MEMPTRl
                               // 1001d MEMPTRh
      fwrite(&tapbyte, 1, 1, fh);   // 1001e tapbyte
      tapstate|= ear<<1 & 128;
      fwrite(&tapstate, 1, 1, fh);  // 1001f tapstate | ear
      fwrite(&block, 2, 1, fh);     // 10020 block
      if( tapstate==1 )
        fwrite(&tapcount, 2, 1, fh);// 10022 tapcount
      else
        fwrite(&tappos, 2, 1, fh);  // 10022 tappos
    }
    fclose(fh);
  }
}