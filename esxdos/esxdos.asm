...
L0032   defb    '/BIN/'         ;avoids clash with keyword in tokenizers
        ...
        defb    $ff
L0038   jr      L001F           ;automatically mapped in by the hardware after M1 when PC=$0038
        ld      hl, $0039
        jp      L1FF4

L0040   defm    'PLUS3DOS'      ;file header text
...
L0050   defm    'Detecting Devices...'
        defm    13, 0           ;carriage return, end marker

NMI     ret                     ;automatically mapped in by the hardware after M1 when PC=$0066
                                ;after triggering the first NMI, the CPU will
...
L00EF   ld      bc, $1ffd       ; select BASIC ROM on 128K machines
...
L0101   ld      bc, $2a30       ;Jumped to from RST0
        xor     a               ;LD A, 0
...
L012A   xor     a               ;start of init
        out     (ula), a        ;make screen black
        ld      hl, $5eff       ;source
...
L013B   out     (mmcram), a     ;set divMMC memory page
        ld      hl, $2000       ;source
...
L0242   ld      a, $7f          ;high byte of I/O address
        in      a, (ula)        ;read keyboard
        rra
        jr      c, L024B
        jr      L0242
...
L0297   ld      b, $ff          ;fixed address used by taps.io
...
L049E   ld      a, c
        ...
        block   $04c6-$
L04C6   ld      hl, $1f80       ; automatically mapped in by the hardware after M1 when PC=004C6h
        push    hl              ; Automapped entry point for SAVE command
        ld      b, a
        xor     a
...
L051A   out     (c), l          ;Set divMMC memory page...
        ld      a, (ix + 0)
...
L065B   out     (c), l          ;set divMMC RAM page...
        ld      a, (ix + 0)
        out     (c), h          ;set divMMC RAM page...
        ld      (de), a
        inc     ix
...
L0670   defm    13, 'Mounting drives...', 13, 13, 0
...
L06BB   defb    $17, $18, $01   ;TAB 24
        defm    "[ERROR]"
        defb    $0d, $00        ;carriage return, end marker

L06C7   defb    $17, $1b, $01   ;TAB 27
        defm    "[OK]"
        defb    $0d, $00        ;carriage return, end marker
...
; called from dirs.io
pr_str  ld      a, (hl)         ;Output a string of characters, zero terminated
        and     a               ;test for zero
...
L0845   push    hl              ; Jumped to from RST10 - print a character in 'a'
        push    de
        push    bc
        push    af
        push    iy
...
L0861   ld      de, $2710       ;called from dirs.io
        call    L087D
        ld      de, $03e8
...
L089A   ld      a, d            ;called from dirs.io
        or      e
        jr      nz, L08AD
        ld      e, h
        ld      h, l
...
L0985   ex      (sp), hl        ;RST08_handler
        ld      ($3dfa), a      ;save parameter in A
        ld      a, (hl)         ;retrieve syscall # from position after RST instruction
        inc     hl              ;adjust return address
        ex      (sp), hl        ;and saves it to the stack

L098C   push    iy
        push    ix
        sub     $80             ;now A holds the syscall number, subtract HOOK_BASE from it
        ld      iyl, a          ;so the syscall number begins now at 0, save in IYl
        ld      ix, (mmc_3)
        xor     a
...
L09E1   cp      '*'             ;use current drive? test for file commands
        ret     z
        ld      c, a
        call    L07C3
        ret     c
...
L0C06   ld      (err_nr), a     ;based on the Spectrum ROM's main_4 / main_g routine
...
pr_msg  ld      a, (hl)         ;called from files.io
        cp      $7f
        push    af
        and     $7f
        rst     $10             ;print a character
...
L0CBD   ld      ($3df2),de      ;jumped from RST $18 (CALLBAS)
        ...
        inc     hl              ;while advancing the return address to skip this value
        ...
        push    de              ;Store the address to call to in system ROM into the stack
...
L0DEB   ld      a, 2            ;called from dirs.io
        rst     $18
        defw    chan_open
        ret
...
L1D81   call    L1D74
        ret     c
        push    af
        call    L1DD2           ;poll the SPI port for a non FFh value to be returned. Result returned in A.
        ld      h, a
        call    L1DD2           ;poll the SPI port for a non FFh value to be returned. Result returned in A.
        ld      l, a
        call    L1DD2           ;poll the SPI port for a non FFh value to be returned. Result returned in A.
...
; Return results in A
L1DD2   ld      bc, $0032       ;number of retries (C)=50*255 (12750)

L1DD5   in      a, (mmcspi)     ;read divMMC SPI port
...
L1FF8   ret                     ; A jump to 1FF8 - 1FFF unmaps divMMC ROM/RAM when M1 goes high
        ei

L1FFA   ret                     ; Jump from RST $18 handler to return to a system ROM routine whose address has 
...
L2488   call    L249A           ;called from dirs.io
        ret     z
        ld      a, b
        or      c
        jp      z, L24EF
...
L24A6   push    af              ;called from dirs.io
        rst     $30
        ld      (bc), a
        cp      $64
        jp      nz, L24EC
...
L24C1   rst     $18             ;called from dirs.io 
        defw    get_char
...
L24CD   ld      sp, (err_sp)    ;called from dirs.io
        ld      (iy + $00), $FF
        ld      hl, $1BF4
        rst     $30
        inc     bc
...
L24E6   ld      a, $02          ;called from files.io
        rst     $20
...
L24F2   ld      a, $04          ;called from files.io
        rst     $20
        
L24F5   ld      a, ($2E32)      ;called from dirs.io
        push    af
        ld      a, $00
        ld      ($2E32), a
        pop     af
...
L2502   ld      a, ($2E32)      ;called from dirs.io
        and     a
        ret     z
        push    bc
        push    de
