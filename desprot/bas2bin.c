...
/*  Modified by Antonio Villena to do headerless                                                  */
/*                                                                                                */
...

{
  ...
  if (FileNameIn[0] == '\0')                                                                         /* We do need an input file! */
    AllOk = FALSE;
  if (!Quiet || !AllOk)
    printf ("\nBAS2BIN v2.5 by Martijn van der Heide/Antonio Villena from BAS2TAP\n\n");
  if (!AllOk)
  {
    printf ("Usage: BAS2BIN [-q] [-w] [-e] [-c] FileIn [FileOut]\n");
  ...
  else if (strcmp (FileNameOut + Size, ".bin") && strcmp (FileNameOut + Size, ".bin"))
    strcpy (FileNameOut + Size, ".bin");
  if (!Quiet)
  ...
  if (!WriteError)                             /* Finish the BIN file no matter what went wrong, unless it was the writing itself */
  {
    if (!Quiet)
    {
      if (AllOk)
