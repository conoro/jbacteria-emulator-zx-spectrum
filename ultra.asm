...
L3C02:  LD      IXH,B
        DEC     (IY+$02)
L3C07:  LD      B,23            ; Dibujar 22 lineas (el primer CR se salta) con "RANDOMIZE " en tinta y fondo negros (se ve una columna negra de 10 caracteres de ancho)
...
L3C0D:  RST     $10             ; Funcion imprime caracter por pantalla, el codigo ASCII se introduce en el registro A
        ...
        DJNZ    L3C0D           ; Repito el bucle 23 veces (dibujo 22 lineas). Si lo hago tras perder una partida se produce scroll (y la tipica pregunta scroll? mas?)
...
L3C1E:  LD      A,R             ; Leo un numero pseudoaleatorio del registro R
L3C20:  SUB     7               ; Estas dos lineas equivalen a un MOD 7
        JR      NC,L3C20        ; o resto de dividir el byte entre 7, ya que necesito elegir una pieza al azar de entre las 7
...
L3C38:  LD      IXL,$EC         ; IX apunta a la subfuncion a llamar (dentro de la funcion L3CD5), en este caso es la que comprueba si hay colision antes de pintar la pieza
        CALL    L3CD5           ; Llamo a la funcion L3CD5 para testear la pieza (con -1 ahorro un byte porque PUSH DE coincide con el ultimo byte de la instruccion anterior)
        JR      Z,L3C4A         ; Si no hay colision, salto a ncol
        ...
        JR      Z,L3C07         ; generado la pieza, en tal caso (con una colision nada mas generar la pieza) reinicio el juego
        INC     C               ; Senalizo la colision poniendo a 1 el bit 1 del registro C
L3C4A:  LD      IXL,$95         ; IX apunta a la subfuncion pintar/borrar
        EX      AF,AF'          ; recupero el color de A'
        CALL    L3CD5           ; pinto la pieza
        ...
        JR      NZ,L3C97        ; Salto a L3C97 en caso de ese tipo de colision
        PUSH    DE              ; Guardo posicion en pila
L3C57:  LD      A,($5C78)       ; Leo contador de frames
        ...
        JR      Z,L3C6C         ; Si se agota el tiempo, la pieza cae por gravedad, salto a "salt"
        BIT     5,(IY+1)        ; Mientras tanto voy leyendo si se ha pulsado una tecla
        JR      Z,L3C57         ; En tal caso, rompo el bucle de tiempo
        LD      A,($5C08)       ; Con el registro A conteniendo el codigo ASCII de la tecla pulsada
L3C6C:  PUSH    HL              ; Guardo pieza en pila
        ...
        CALL    L3CD5           ; Borra pieza
        POP     AF              ; Recupero tecla pulsada
        SUB     $4F             ; He pulsado izquierda?
        JR      NZ,L3C7B        ; No, pues salto y no hago nada
        DEC     E               ; Si, pues decremento posicion
L3C7B:  DEC     A               ; He pulsado derecha? seria caracter 'p' justo despues de 'o', por eso basta con un decremento para comparar
        JR      NZ,L3C7F        ; No, pues salto y no hago nada
        INC     E               ; Si, pues incremento posicion
L3C7F:  DEC     A               ; He pulsado arriba (rotar)? seria caracter 'q', despues de 'p'
        LD      C,1             ; Inicializo A y C a cero y uno respectivamente, independientemente de si salto o no
        JR      Z,L3CC4         ; Si, pues salto a rota (con A y C inicializadas)
...
L3C87:  LD      BC,$2004        ; Inicializo B a 32 (bajo posicion una fila completa) y C a 4 indicando que entro al bucle principal via pieza no acelerada
        JR      C,L3C38         ; Si la pieza cae por su peso (ninguna tecla pulsada), cierro bucle principal
        INC     C               ; Si se ha pulsado 'a' o equivalente, senalizo pieza acelerada en registro C
L3C8D:  INC     DE              ; Avanza la posicion en una fila (32 caracteres)
        DJNZ    L3C8D
        LD      (IY),A          ; Pongo FRAMES1 (antes guardada en A) como referencia en time (variable incrustada en codigo)
        JR      L3C38           ; Cierro bucle principal
        LD      (DE),A          ; Una de las dos opciones de subfuncion a llamar (En este caso pinta/borra pieza)
        RET
L3C97:  LD      HL,$5ABF        ; Parto desde una coordenada (31, 21) que es la parte inferior derecha de la pantalla
L3C9A:  EX      DE,HL           ; Almaceno posicion en DE
        ...
        JR      Z,L3C9A         ; Bucle que voy repitiendo (probando con lineas que estan por encima) hasta detectar una falsa linea completa en la ROM (debajo de $4000)
        ...
        LD      A,H
        SUB     $58
        JP      C,L3C1E         ; Si me salgo de la zona de atributos es que ya he llegado a la primera linea y por tanto salgo del bucle (a generar una nueva pieza)
        LD      B,A
        ...
        JR      L3C97           ; Cierro bucle
L3CBD:  DJNZ    L3CC7           ; Repito bucle interior 4 veces
        POP     HL              ; Recupero HL de pila
        RR      H               ; Desplazo HL hacia la derecha
        RR      L
L3CC4:  LD      B,4             ; Pongo contador a 4
        PUSH    HL              ; Guardo HL en pila, ya que no quiero que pierda su valor al desplazarlo 4 veces
L3CC7:  ADD     HL,HL           ; Desplazo 4 veces HL a la izquierda
        ADD     HL,HL
        ADD     HL,HL
        ADD     HL,HL
        RL      C               ; Propago el contenido al par de registros (A, C)
        RLA
        JR      NC,L3CBD        ; Como inicialmente (A, C) valia 1, esto me indica que he llegado al bit marcador, por tanto ya he movido 16 bits
        ...
        LD      L,C
        JR      L3C87           ; Salto al bucle principal (con indicador de pieza no acelerada)
L3CD5:  PUSH    DE              ; Guardo DE, HL y BC en pila
        PUSH    HL
        PUSH    BC
L3CD8:  LD      B,4             ; El bucle interior es de 4 y el exterior es de C, por tanto se repite 4xC veces, siendo C siempre mayor que 4. Como HL no puede tener mas de 16 bits solo se pinta/borra/comprueba colision en una reticula de 4x4
L3CDA:  ADD     HL,HL           ; Siguiente bit dentro de la reticula 4x4 a comparar
        ...
        DJNZ    L3CDA           ; Cierro bucle interior
...
L3CE3:  DEC     DE              ; Hago la resta via bucle con djnz, que equivaldria a SUB DE, 28
        DJNZ    L3CE3
        DEC     C               ; Cierro bucle exterior
        JR      NZ,L3CD8
L3CE9:  POP     BC              ; Recupero BC, HL y DE de la pila
        JP      $1A45
        ...
        POP     DE              ; Si no es negro, hay colision, lo indico con carry Z desactivo y salgo de la subfuncion y de la funcion L3CD5
        JR      L3CE9

L3CF2:  DEFB    $FF, $FF, $FF, $FF, $FF, $FF, $FF; 7 bytes
