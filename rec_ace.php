...
if( $_POST['enviar'] ){
  require 'connect.php';
  list($name_year, $publisher)= $db->query('SELECT CONCAT(name," ",year), publisher FROM ace_games WHERE shortname="'.$_POST['sname'].'"')->fetch_row();
  $db->query('INSERT ace_record VALUES("'.$_POST['id'].'","'.
             $_POST['url'].'",'.
             $_POST['frames'].',"'.
             $name_year.'","'.
             $publisher.'",'.
...
while ( $len<0xe000 ){
  $snap.= chr($value= getval($index, $str));
  if( $value == 0xed ){
    $snap.= chr($value= getval($index, $str));
    $len+= $value;
...
file_put_contents('snaps/'.$b64.'.ace.deflate', gzdeflate($snap));
?><pre style="text-align:center;font-size:20px">The recorded gameplay is located at:
<a href="<?=$url?>"><?=$url?></a>


Also you can <a href="<?=$urls.'?'.$b64.'.ace'?>">play</a> or <a href="snaps/<?=$b64.'.ace'?>">download</a> the final snapshot.

<form method="post" action="rec_ace.php">
